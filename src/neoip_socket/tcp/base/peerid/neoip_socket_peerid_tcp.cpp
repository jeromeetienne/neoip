/*! \file
    \brief definition of the \ref socket_peerid_tcp_t
*/
...
FACTORY_PRODUCT_DEFINITION(socket_peerid_vapi_t, socket_peerid_tcp_t);
 ...
 */
socket_err_t	socket_peerid_tcp_t::ctor_from_str(const std::string &str)	throw()
{
	// parse the input
	ipaddr	= ip_addr_t(str.c_str());
	// if an error occur while parsing this, report it now
	if( ipaddr.is_null() )
		return socket_err_t(socket_err_t::BAD_PARAM, "bad socket_peerid_tcp_t format in "+ str);
	// return no error
 ...
 */
socket_domain_t socket_peerid_tcp_t::get_domain()	const throw()
{
 ...
 */
bool	socket_peerid_tcp_t::is_localhost()			const throw()
{
 ...
 */
bool	socket_peerid_tcp_t::is_linklocal()			const throw()
{
 ...
 */
bool	socket_peerid_tcp_t::is_private()			const throw()
{
 ...
 */
bool	socket_peerid_tcp_t::is_public()			const throw()
{
 ...
 */
bool	socket_peerid_tcp_t::is_any()			const throw()
{
...
/** \brief compare 2 \ref socket_peerid_tcp_t
 */
int	socket_peerid_tcp_t::compare(const socket_peerid_vapi_t &other_api)	const throw()
{
	// sanity check - both MUST have the same type
	DBG_ASSERT( typeid(*this) == typeid(other_api) );
	// cast the base class into the derived one
	const socket_peerid_tcp_t &other = dynamic_cast <const socket_peerid_tcp_t&>(other_api);
	// compare the ip addr
 ...
 */
std::string	socket_peerid_tcp_t::to_string()				const throw()
{
 ...
 */
void	socket_peerid_tcp_t::serialize(serial_t &serial)			const throw()
{
	serial << ipaddr;
}
 ...
 */
void	socket_peerid_tcp_t::unserialize(serial_t &serial)			throw(serial_except_t)
{
	serial >> ipaddr;
}
