/*! \file
    \brief Declaration of the neoip_socket_udp_full
*/
...
FACTORY_PRODUCT_DEFINITION(socket_full_vapi_t, socket_full_udp_t);
...
/** \brief Constructor
 */
socket_full_udp_t::socket_full_udp_t()					throw()
{
	// log to debug
	KLOG_DBG("enter");
	// zero from field
	...
	socket_layer_udp_get()->socket_full_dolink(this);
}
...
socket_full_udp_t::~socket_full_udp_t()					throw()
{
	...
	KLOG_DBG("enter");
	// link this object to the socket_layer_udp_t
	socket_layer_udp_get()->socket_full_unlink(this);
 ...
 * - this function attachs the socket_full_udp_t to a socket_full_close_udp_t
 */
void socket_full_udp_t::notify_glue_destruction()		throw()
{
	// TODO here issue if the socket_full_udp_t has not yet been started
	// pass the socket_full_vapi_t in socket_full_close_t (up to it to destroy it when appropriate)
	nipmem_new socket_full_close_udp_t(this);
}

/** \brief start the closure 
 * 
 * - called only by the socket_full_close_udp_t
 */
void socket_full_udp_t::start_closure()	throw() 
{
 ...
 */
void	socket_full_udp_t::set_callback(socket_full_vapi_cb_t *callback, void *userptr)	throw()
{
 ...
 * 
 * - used only by socket_resp_udp_t and socket_itor_udp_t
 ...
 */
socket_err_t	socket_full_udp_t::set_ctor_param(udp_full_t *udp_full, nlay_full_t *nlay_full
...
/** \brief to access nlay_full_upapi_t of this socket_full_udp_t
 */
const nlay_full_upapi_t &	socket_full_udp_t::nlay()	const throw()
{
...
/** \brief to access nlay_full_upapi_t of this socket_full_udp_t
 */
nlay_full_upapi_t &	socket_full_udp_t::nlay()		throw()
{
	// sanity check nlay_full MUST be non null
	DBG_ASSERT( nlay_full );
	// return the nlay_scnx_full
 ...
 */
bool	socket_full_udp_t::neoip_inet_udp_full_event_cb(void *userptr, udp_full_t &cb_udp_full
					, const udp_event_t &udp_event)		throw()
{
	...
	case udp_event_t::CNX_CLOSED:{
			// build the event
			std::string	reason	= udp_event.get_cnx_closed_reason();
	...
	case udp_event_t::MTU_CHANGE:{
			size_t	new_outter_mtu	= udp_event.get_mtu_change();
 ...
 */
bool	socket_full_udp_t::neoip_nlay_full_event_cb(void *userptr, nlay_full_t &cb_nlay_full
					, const nlay_event_t &nlay_event)	throw()
{
	...
	// sanity check - the udp_full and nlay_itor MUST NOT be null
	DBG_ASSERT( nlay_full );
	DBG_ASSERT( udp_full );
	...
	case nlay_event_t::CNX_DESTROYED:{
			// sanity check - this event MUST be reported only to socket_full_close_udp_t
			DBG_ASSERT( typeid(*callback) == typeid(socket_full_close_udp_t));
			// notify a CNX_CLOSED 
	...
	case nlay_event_t::PKT_TO_LOWER:
			// send the packet thru the udp_full
			udp_full->send( *nlay_event.get_pkt_to_lower() );
			break;
...
void	socket_full_udp_t::rcvdata_maxlen(size_t new_value)	throw()
{
...
size_t	socket_full_udp_t::rcvdata_maxlen()			const throw()
{
	// sanity check - the inner type MUST be a reliable one
	DBG_ASSERT( type().is_reliable() );
	// forward the function to nlay_full_t
...
void	socket_full_udp_t::maysend_tshold(size_t new_value)	throw()
{
	// forward the function to nlay_full_t
	nlay_err_t	nlay_err;
...
size_t	socket_full_udp_t::maysend_tshold()			const throw()
{
	if( nlay_full->maysend_is_set() )	return socket_full_t::UNLIMITED;
	return nlay_full->maysend_get_threshold();
}

void	socket_full_udp_t::xmitbuf_maxlen(size_t new_value)	throw()
{
	nlay_err_t	nlay_err;
...
size_t	socket_full_udp_t::xmitbuf_maxlen()			const throw()
{
	if( nlay_full->sendbuf_is_limited() )	return socket_full_t::UNLIMITED;
	return nlay_full->sendbuf_get_max_len();
}

size_t	socket_full_udp_t::xmitbuf_usedlen()			const throw()
{
	return nlay_full->sendbuf_get_used_len();
}

size_t	socket_full_udp_t::xmitbuf_freelen()			const throw()
{
	if( nlay_full->sendbuf_is_limited() )	return socket_full_t::UNLIMITED;
...
void	socket_full_udp_t::mtu_pathdisc(bool onoff)	throw()
{
	udp_full->mtu_pathdisc(onoff);	
}

bool	socket_full_udp_t::mtu_pathdisc()		const throw()
{
	return udp_full->mtu_pathdisc();
}

size_t	socket_full_udp_t::mtu_overhead()		const throw()
{
	return udp_full->mtu_overhead() + nlay_full->mtu_overhead();
}

size_t	socket_full_udp_t::mtu_outter()		const throw()
{
	return udp_full->mtu_outter();
}

size_t	socket_full_udp_t::mtu_inner()		const throw()
{
	return udp_full->mtu_outter() - mtu_overhead();	
}
 ...
 */
std::string	socket_full_udp_t::to_string()					const throw()
{
 ...
 */
bool	socket_full_udp_t::notify_callback(const socket_event_t &socket_event)	throw()
{
