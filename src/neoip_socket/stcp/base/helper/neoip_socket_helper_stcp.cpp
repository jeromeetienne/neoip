...
// definition of \ref socket_helper_stcp_t constant
const socket_domain_t	socket_helper_stcp_t::DOMAIN_VAR	= strtype_socket_domain_t::STCP;
 ...
 * 
 */
tcp_profile_t	socket_helper_stcp_t::socket_to_tcp_profile(const socket_profile_t &socket_profile) throw()
{
 ...
 */
ipport_addr_t	socket_helper_stcp_t::ipport_addr(const socket_addr_t &socket_addr)	throw()
{
	// sanity check - socket_addr MUST be of the proper socket_domain_t
	DBG_ASSERT( socket_addr.get_domain() == socket_domain_t::STCP );
	DBG_ASSERT( typeid(*socket_addr.get_peerid_vapi()) == typeid(socket_peerid_stcp_t));
	DBG_ASSERT( typeid(*socket_addr.get_portid_vapi()) == typeid(socket_portid_stcp_t));
	// get the ip_addr_t from the socket_peerid_t
	ip_addr_t	ip_addr	= (dynamic_cast <const socket_peerid_stcp_t &>(*socket_addr.get_peerid_vapi())).ipaddr;
	// get the port from the socket_portid_t
	uint16_t	port	= (dynamic_cast <const socket_portid_stcp_t &>(*socket_addr.get_portid_vapi())).port;
...
/** \brief Populate the socket_*_t factory with all the socket_domain_t::STCP class
 */
void	socket_helper_stcp_t::factory_domain_insert()				throw()
{
	// insert all the factory_product_t for strtype_socket_domain_t::STCP
	FACTORY_PRODUCT_INSERT(socket_profile_factory, socket_domain_t, socket_profile_vapi_t
				, strtype_socket_domain_t::STCP, socket_profile_stcp_t);
	FACTORY_PRODUCT_INSERT(socket_peerid_factory, socket_domain_t, socket_peerid_vapi_t
				, strtype_socket_domain_t::STCP, socket_peerid_stcp_t);
	FACTORY_PRODUCT_INSERT(socket_portid_factory, socket_domain_t, socket_portid_vapi_t
				, strtype_socket_domain_t::STCP, socket_portid_stcp_t);
	FACTORY_PRODUCT_INSERT(socket_resp_factory, socket_domain_t, socket_resp_vapi_t
				, strtype_socket_domain_t::STCP, socket_resp_stcp_t);
	FACTORY_PRODUCT_INSERT(socket_itor_factory, socket_domain_t, socket_itor_vapi_t
				, strtype_socket_domain_t::STCP, socket_itor_stcp_t);
	FACTORY_PRODUCT_INSERT(socket_full_factory, socket_domain_t, socket_full_vapi_t
				, strtype_socket_domain_t::STCP, socket_full_stcp_t);
}
