/*! \file
    \brief Class to handle the bt_httpi_t

\par Brief Description
bt_httpi_t reads data from a http connection (no requirement on seekability)
...
- the bt_httpi_t::cur_offset() is the one at which the chunk starts
  - aka the length of the chunk is updated AFTER the callback notification
  
\par About the circularidx
- bt_httpi_t do not handle any of the circularidx, it is handled by
  the bt_io_pfile_t
 ...
 */
bt_httpi_t &	bt_httpi_t::set_profile(const bt_httpi_profile_t &profile)	throw()
{
				...
				, bt_httpi_cb_t *callback, void *userptr) 	throw()
{
	...
	wikidbg_obj_add_static_page("/bt_httpi_" + OSTREAMSTR(this));
 ...
 */
bool	bt_httpi_t::neoip_http_client_cb(void *cb_userptr, http_client_t &cb_http_client
 ...
 */
bool	bt_httpi_t::neoip_socket_full_event_cb(void *userptr, socket_full_t &cb_socket_full
 ...
 */
bool	bt_httpi_t::handle_recved_data(pkt_t &pkt)	throw()
{
	// log to debug
	...
	bt_io_write	= io_vapi()->write_ctor(file_range, pkt.to_datum(datum_t::NOCOPY)
							, this, NULL);
 ...
 */
bool	bt_httpi_t::neoip_bt_io_write_cb(void *cb_userptr, bt_io_write_t &cb_io_write
