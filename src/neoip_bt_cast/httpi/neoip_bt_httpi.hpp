/*! \file
    \brief Header of the bt_httpi_t
    
*/


#ifndef __NEOIP_BT_HTTPI_HPP__ 
#define __NEOIP_BT_HTTPI_HPP__ 
/* system include */
/* local include */
...
/** \brief class definition for bt_httpi
 */
class bt_httpi_t : NEOIP_COPY_CTOR_DENY, private http_client_cb_t, private socket_full_cb_t
		, private bt_io_write_cb_t
		, private wikidbg_obj_t<bt_httpi_t, bt_httpi_wikidbg_init>  {
private:
	http_uri_t		m_http_uri;	//!< the source of the http stream
	bt_io_vapi_t *		m_io_vapi;	//!< the bt_io_vapi_t
	file_size_t		m_cur_offset;	//!< the amount of byte already written
	bt_httpi_profile_t	profile;	//!< the profile to use for this bt_httpi_t
	
	/*************** bt_httpi_mod_vapi_t	*******************************/
	bt_httpi_mod_vapi_t *	m_mod_vapi;	//!< pointer on the bt_httpi_mod_vapi_t
	bt_err_t		mod_vapi_ctor(const bt_httpi_mod_type_t &mod_type)	throw();
	bool			mod_vapi_notify_callback(const bt_httpi_event_t &event)	throw();
	...
	/*************** callback stuff	***************************************/
	bt_httpi_cb_t *		callback;	//!< callback used to notify result
	void *			userptr;	//!< userptr associated with the callback
	bool			notify_callback_failed(const bt_err_t &bt_err)		throw();
	bool			notify_callback(const bt_httpi_event_t &httpi_event)	throw();
...
public:
	...
	/*************** setup function	***************************************/
	bt_httpi_t &		set_profile(const bt_httpi_profile_t &profile)	throw();
	bt_err_t		start(const http_uri_t &m_http_uri, bt_io_vapi_t *m_io_vapi
	...
	bt_httpi_mod_vapi_t *	mod_vapi()	const throw()	{ return m_mod_vapi;	}
	
	/*************** List of friend class	*******************************/
	friend class	bt_httpi_wikidbg_t;
	friend class	bt_httpi_mod_flv_t;
	friend class	bt_httpi_mod_raw_t;
};

NEOIP_NAMESPACE_END

#endif	/* __NEOIP_BT_HTTPI_HPP__  */
