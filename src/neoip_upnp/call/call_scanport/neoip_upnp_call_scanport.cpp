/*! \file
    \brief Definition of the upnp_call_scanport_t

\par About upnp_disc_res_t and local revendian field
- upnp_call_scanport_t will use the reverse endianness bug workaround IIF revendian() is set
...
- thus if the caller want to force the usage of revendian() even if
  upnp_disc_res_t::getport_endianbug(), it may use call_scanport->revendian(true).start(...)
...
/** \brief Constructor
 */
upnp_call_scanport_t::upnp_call_scanport_t()		throw()
{
 ...
 */
upnp_call_scanport_t &upnp_call_scanport_t::set_profile(const upnp_call_profile_t &profile)	throw()
{
 ...
 */
upnp_err_t upnp_call_scanport_t::start(const upnp_disc_res_t &disc_res, uint32_t scanidx
				, upnp_call_scanport_cb_t *callback, void * userptr)	throw()
{
	...
	std::string	method_name	= "GetGenericPortMappingEntry";
	strvar_db_t	strvar_db;
	strvar_db.append("NewPortMappingIndex"	, OSTREAMSTR(scanidx)		);
	// launch the upnp_call_t
	upnp_err	= upnp_call->start(disc_res.control_uri(), disc_res.service_name()
						, method_name, strvar_db, this, NULL);
 ...
 */
bool	upnp_call_scanport_t::neoip_upnp_call_cb(void *cb_userptr, upnp_call_t &cb_call
 ...
 */
bool upnp_call_scanport_t::notify_callback(const upnp_err_t &upnp_err)	throw()
{
	...
	// notify the caller
	bool tokeep = callback->neoip_upnp_call_scanport_cb(userptr, *this, upnp_err);
