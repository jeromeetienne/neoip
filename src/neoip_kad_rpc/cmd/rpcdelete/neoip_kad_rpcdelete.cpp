/*! \file
    \brief Definition of the \ref kad_rpcdelete_t class
...
/** \brief Constructor
 */
kad_rpcdelete_t::kad_rpcdelete_t()		throw()
{
	// zero some field
	m_rpcpeer	= NULL;
	m_kad_delete	= NULL;
...
kad_rpcdelete_t::~kad_rpcdelete_t()		throw()
{
	...
	if( m_rpcpeer )	rpcpeer()->rpcdelete_unlink(this);
	// delete the kad_delete_t if needed
	nipmem_zdelete	m_kad_delete;
}
 ...
 */
kad_err_t	kad_rpcdelete_t::start(kad_rpcresp_t *kad_rpcresp, const httpd_request_t &recved_request
							...
							, xmlrpc_parse_t &xmlrpc_parse)	throw()
{
	...
	try {
		...
		xmlrpc_parse >> xmlrpc_parse_t::PARAM_BEG >> recid		>> xmlrpc_parse_t::PARAM_END;
	...
	rpcpeer()->rpcdelete_dolink(this);
	
	// log to debug
	KLOG_ERR("rpcpeer_id="		<< rpcpeer_id);
	KLOG_ERR("recid="		<< recid);
	KLOG_ERR("keyid="		<< keyid);
	KLOG_ERR("expire_delay="	<< expire_delay);

	// start the kad_delete_t
	m_kad_delete	= nipmem_new kad_delete_t();
	kad_err		= kad_delete()->start(rpcpeer()->kad_peer(), recid, keyid, expire_delay, this, NULL);
 ...
 */
bool kad_rpcdelete_t::neoip_kad_delete_cb(void *cb_userptr, kad_delete_t &cb_kad_delete
