...
/* system include */
/* local include */
#include "neoip_btrelay_info.hpp"
...
int main_internal(int argc, char **argv)
{
	...
	libsess_err_t	libsess_err	= lib_apps->start(argc, argv, btrelay_apps_t::clineopt_arr()
	...
	// launch the btrelay_apps_t
	lib_session_exit_ptr_t<btrelay_apps_t> btrelay_apps(nipmem_new btrelay_apps_t(), lib_session_exit_t::EXIT_ORDER_APPS);
	bt_err_t	bt_err		= btrelay_apps->start();
	if( bt_err.failed() ){
	...
	// delete btrelay_apps_t
	btrelay_apps.delete_ptr();
