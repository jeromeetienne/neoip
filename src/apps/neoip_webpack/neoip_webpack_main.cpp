/*! \file
    \brief main() for neoip-webpack

\par Brief Description
This application is used to export all the functionality of libneoip_webpack into
a commandline tool
...
/* system include */
/* local include */
#include "neoip_webpack_info.hpp"
...
int main_internal(int argc, char **argv)
{
	...
	libsess_err_t	libsess_err	= lib_apps->start(argc, argv, webpack_apps_t::clineopt_arr()
	...
	// launch the webpack_apps_t
	lib_session_exit_ptr_t<webpack_apps_t> webpack_apps(nipmem_new webpack_apps_t(), lib_session_exit_t::EXIT_ORDER_APPS);
	bt_err_t	bt_err	= webpack_apps->start();
	if( bt_err.failed() ){
	...
	// log the event
	KLOG_WARN("NeoIP WEBPACK Application Initialized");
	// Start the event loop - exit only when the apps goes off
	lib_session_get()->loop_run();

	// delete webpack_apps_t 
	webpack_apps.delete_ptr();
