...
#include "neoip_flv2xml_info.hpp"
...
int main(int argc, char **argv)
{
	...
	libsess_err_t	libsess_err	= lib_apps->start(argc, argv, flv2xml_apps_t::clineopt_arr()
	...
	// launch the flv2xml_apps_t
	lib_session_exit_ptr_t<flv2xml_apps_t>	flv2xml_apps(nipmem_new flv2xml_apps_t(), lib_session_exit_t::EXIT_ORDER_APPS);
	flv_err_t	flv_err		= flv2xml_apps->start();
	if( flv_err.failed() ){
	...
	// delete flv2xml_apps_t 
	flv2xml_apps.delete_ptr();
