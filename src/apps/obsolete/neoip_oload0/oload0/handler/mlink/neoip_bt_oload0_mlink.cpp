/*! \file
    \brief Definition of the \ref bt_oload0_mlink_t

\par Brief Description
\ref bt_oload0_mlink_t is a bt_oload0_t handler which receives a http_uri_t pointing
as a .metalink file. It download the .metalink file via bt_oload0_mlink_itor_t, it
may do some other http to get the file length,and then start downloading on the 
swarm and forward it to all the attached bt_httpo_full_t.

- NOTE: this seems to works but it is kludgy in many places
  - the mlink_file_t parsing is so-so
  - the bt_oload0_mlink_itor_t is ugly and inefficient at best
  - apparently it trigger some issue in the http too :)
...
/** \brief Constructor
 */
bt_oload0_mlink_t::bt_oload0_mlink_t()	throw()
{
	// log to debug
	KLOG_DBG("enter");
	// zero some field
 ...
 */
bt_oload0_mlink_t &bt_oload0_mlink_t::set_profile(const bt_oload0_mlink_profile_t &profile)	throw()
{
 ...
 */
bt_err_t	bt_oload0_mlink_t::start(bt_oload0_t *bt_oload0)			throw()
{
	bt_err_t	bt_err;
	// log to debug
	KLOG_DBG("enter");
	...
	bt_err		= httpo_resp->start(bt_oload0->get_bt_httpo(), "http://0.0.0.0/metalink"
...
/** \brief Return a pointer to a bt_oload0_mlink_itor_t matching the nested_uri, or NULL if none matches
 */
bt_oload0_mlink_itor_t *	bt_oload0_mlink_t::itor_by_nested_uri(const http_uri_t &nested_uri) const throw()
{
	std::list<bt_oload0_mlink_itor_t *>::const_iterator	iter;
	// go thru the whole torr_itor_db
	for(iter = torr_itor_db.begin(); iter != torr_itor_db.end(); iter++){
		bt_oload0_mlink_itor_t *	stat_itor	= *iter;
		// if this bt_oload0_mlink_itor_t matches this nested_uri, return its pointer
 ...
 */
bool	bt_oload0_mlink_t::neoip_bt_httpo_resp_cb(void *cb_userptr, bt_httpo_resp_t &cb_bt_httpo_resp
						, const bt_httpo_event_t &httpo_event)	throw()
{
	bt_oload0_swarm_t *	oload0_swarm;
	bt_oload0_mlink_itor_t *	stat_itor;
	// log to debug
	...
	bt_err_t	bt_err;
	stat_itor	= nipmem_new bt_oload0_mlink_itor_t();
