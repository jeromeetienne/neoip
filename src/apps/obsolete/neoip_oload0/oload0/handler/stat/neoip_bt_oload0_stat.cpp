/*! \file
    \brief Definition of the \ref bt_oload0_stat_t
...
/** \brief Constructor
 */
bt_oload0_stat_t::bt_oload0_stat_t()	throw()
{
	// log to debug
	KLOG_DBG("enter");
	// zero some field
...
bt_oload0_stat_t::~bt_oload0_stat_t()	throw()
{
	// log to debug
	KLOG_DBG("enter");
	// delete all the stat_itor_db if needed
	while( !stat_itor_db.empty() )	nipmem_delete stat_itor_db.front();
 ...
 */
bt_oload0_stat_t &bt_oload0_stat_t::set_profile(const bt_oload0_stat_profile_t &profile)	throw()
{
 ...
 */
bt_err_t	bt_oload0_stat_t::start(bt_oload0_t *bt_oload0)			throw()
{
	bt_err_t	bt_err;
	// log to debug
	KLOG_DBG("enter");
	...
	bt_err		= httpo_resp->start(bt_oload0->get_bt_httpo(), "http://0.0.0.0/static"
...
/** \brief Return a pointer to a bt_oload0_stat_itor_t matching the nested_uri, or NULL if none matches
 */
bt_oload0_stat_itor_t *	bt_oload0_stat_t::itor_by_nested_uri(const http_uri_t &nested_uri) const throw()
{
	std::list<bt_oload0_stat_itor_t *>::const_iterator	iter;
	// go thru the whole stat_itor_db
	for(iter = stat_itor_db.begin(); iter != stat_itor_db.end(); iter++){
		bt_oload0_stat_itor_t *	stat_itor	= *iter;
		// if this bt_oload0_stat_itor_t matches this nested_uri, return its pointer
 ...
 */
bool	bt_oload0_stat_t::neoip_bt_httpo_resp_cb(void *cb_userptr, bt_httpo_resp_t &cb_bt_httpo_resp
						, const bt_httpo_event_t &httpo_event)	throw()
{
	bt_oload0_swarm_t *	oload0_swarm;
	bt_oload0_stat_itor_t *	stat_itor;
	// log to debug
	...
	bt_err_t	bt_err;
	stat_itor	= nipmem_new bt_oload0_stat_itor_t();
