...
#ifndef __NEOIP_BT_JAMSTD_RESP_HPP__ 
#define __NEOIP_BT_JAMSTD_RESP_HPP__ 
/* system include */
/* local include */
#include "neoip_bt_jamstd_resp_wikidbg.hpp"
#include "neoip_bt_jamstd_resp_state.hpp"
 ...
 */
class bt_jamstd_resp_t : NEOIP_COPY_CTOR_DENY
			, private wikidbg_obj_t<bt_jamstd_resp_t, bt_jamstd_resp_wikidbg_init> {
private:
	...
	bt_jamstd_resp_state_t	m_state;	//!< the current state
	...
	skey_ciph_t *		m_xmit_ciph;	//!< skey_ciph_t to encrypt m_xmit_buffer
	...
	uint16_t		m_initpkt_totlen;//!< the length of the initpkt (used in INITPKT_WAIT)
	...
	bt_err_t		parse_in_padauth_wait(bt_jamstd_full_t **jamstd_full_out
								, bool *stop_parsing)	throw();
	bt_err_t		parse_in_initpkt_wait(bt_jamstd_full_t **jamstd_full_out
								, bool *stop_parsing)	throw();

	/*************** private function	*******************************/
	pkt_t			build_cnxesta_pkt()					throw();
	pkt_t			build_cnxauth_pkt()					throw();
public:
	/*************** ctor/dtor	***************************************/
	bt_jamstd_resp_t()	throw();
	~bt_jamstd_resp_t()	throw();

	/*************** Setup function	***************************************/
	bt_jamstd_resp_t&	profile(const bt_jamstd_profile_t &profile)		throw();
	...
	friend class	bt_jamstd_resp_wikidbg_t;
};


NEOIP_NAMESPACE_END

#endif	/* __NEOIP_BT_JAMSTD_RESP_HPP__  */
