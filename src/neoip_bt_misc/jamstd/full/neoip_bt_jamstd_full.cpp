 ...
 */
bt_jamstd_full_t::bt_jamstd_full_t()				throw()
{
	// zero some fields
	m_recv_ciph	= NULL;
	m_xmit_ciph	= NULL;
 ...
 * 
 * - m_recv_ciph/m_xmit_ciph points to the skey_ciph_t used for encrypt/decrypt
 *   - they may be NULL if no encryption is done
 * - m_init_recvbuf is the initial data recved during the connection establishement
 */
bt_err_t bt_jamstd_full_t::start(skey_ciph_t *m_recv_ciph, skey_ciph_t *m_xmit_ciph
 ...
 */
bt_err_t	bt_jamstd_full_t::notify_recved_data(const pkt_t &pkt_inp, pkt_t &pkt_out)throw()
{
	// log to debug
	KLOG_DBG("enter");
 ...
 */
bt_err_t	bt_jamstd_full_t::notify_toxmit_data(const pkt_t &pkt_inp, pkt_t &pkt_out)throw()
{
	// log to debug
	KLOG_DBG("enter");
	// - do a zerocopy assignment of pkt_inp data into pkt_out 
