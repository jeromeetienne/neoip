/*! \file
    \brief Declaration of the slay_itor_btjamrc4_t
*/
...
FACTORY_PRODUCT_DEFINITION(slay_itor_vapi_t, slay_itor_btjamrc4_t);
...
/** \brief Constructor
 */
slay_itor_btjamrc4_t::slay_itor_btjamrc4_t()						throw()
{
	// init some fields
 ...
 */
slay_err_t	slay_itor_btjamrc4_t::start(const slay_itor_arg_t &itor_arg)		throw()
{
	...
	// set the tls_profile_t to m_btjamrc4_itor
	const slay_profile_btjamrc4_t & dom_profile	= slay_profile_btjamrc4_t::from_slay(itor_arg.profile());
	m_btjamrc4_itor.profile( dom_profile.bt_jamrc4() );
	
	// start the bt_jamrc4_itor_t
	bt_err_t	bt_err;
	bt_err		= m_btjamrc4_itor.start();
 ...
 */
slay_err_t	slay_itor_btjamrc4_t::notify_recved_data(const pkt_t &pkt_inp
						, slay_full_t **slay_full_out)	throw()
{
	...
	*slay_full_out	= NULL;
	...
	// notify pkt_inp to m_btjamrc4_itor
	bt_err	= m_btjamrc4_itor.notify_recved_data(pkt_inp, &btjamrc4_full);
	if( bt_err.failed() )	return slay_err_from_bt(bt_err);
	
	// if bt_jamrc4_full_t is replied by m_btjamrc4_itor, build the slay_full_t for it
	if( btjamrc4_full ){
 ...
 */
std::string	slay_itor_btjamrc4_t::to_string()					const throw()
{
	std::ostringstream	oss;
	// build the string
	oss << "slay_itor_btjamrc4_t";
