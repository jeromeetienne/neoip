/*! \file
    \brief Implementation of \ref file_path2_t to read directories
...
// definition of \ref nunit_path2_t constant
const std::string	file_path2_t::FS_DIR_SEPARATOR		= "/";
const std::string	file_path2_t::FS_DIR_PLACEHOLDER	= ".";
const std::string	file_path2_t::FS_DIR_PARENT		= "..";
 ...
 */
file_path2_t::file_path2_t(const char * path2_ptr)		throw()
{
	// if the path2_ptr is NULL, return now (and leave a null object)
	if( !path2_ptr )	return;
	// init the object
	ctor_from_str(path2_ptr);
}

/** \brief Constructor from std::string
 */
file_path2_t::file_path2_t(const std::string &path2name)		throw()
{
	// if the path2name is empty, return now (and leave a null object)
	if( path2name.empty() )	return;
	// init the object
	ctor_from_str(path2name);
}
 ...
 */
bool	file_path2_t::is_drvletter(const std::string &str)			throw()
{
 ...
 */
void file_path2_t::ctor_from_str(const std::string &path2name)	throw()
{
	// log to debug
	KLOG_DBG("enter path2name=" << path2name);

	// extract the path from the string
	if( !path2name.empty() ){
		name_db = string_t::split(path2name, "/\\");
	}else{
...
/** \brief normalize the file_path2_t
 */
void	file_path2_t::normalize()	throw()
{
	...
	// it it is a absolute path2, remove any FS_DIR_PARENT which follow it
...
/** \brief Append a file_path2_t to another
 */
file_path2_t &	file_path2_t::operator /=(const file_path2_t &other)	throw()
{
 ...
 */
bool	file_path2_t::is_null()		const throw()
{
...
/** \brief Return true if the file_path2_t is absolute, false otherwise
 */
bool	file_path2_t::is_absolute()	const throw()
{
	// sanity check - the path2 MUST be initialized
	DBG_ASSERT( !is_null() );
	
	// if the first name is empty, it is an absolute path2
	if( name_db[0] == "" )	return true;
	// else it isnt
	return false;	
}

/** \brief Return true if this file_path2_t contains of the other file_path2_t
 * 
 * - it return true is if this file_path2_t and the other one are equal or the 
 *   other one is a subpath of this one
 *   - e.g. /tmp/bla is a subpath of /tmp
 */
bool	file_path2_t::contain(const file_path2_t &other)	const throw()
{
	// if any of the two file_path2_t is null, return false
	if( is_null() || other.is_null() )	return false;
	// if the other file_path2_t has less name than this one, return false
	if( other.size() < size() )		return false;
	// go thru all the element of this file_path2_t
	// - purposely ignoring any additionnaly elements in the other file_path2_t
 ...
 */
std::string	file_path2_t::basename_ext()	const throw()
{
 ...
 *   - if the basename contains no dot, it returns a empty string
 */
std::string	file_path2_t::basename_noext()	const throw()
{
	// sanity check - the file_path2_t MUST NOT be null
	DBG_ASSERT( !is_null() );
	// Extract the basename
 ...
 */
file_path2_t	file_path2_t::basename()	const throw()
{
	// sanity check - the file_path2_t MUST NOT be null
	DBG_ASSERT( !is_null() );
	// Extract the basename
 ...
 */
int file_path2_t::compare(const file_path2_t &other)	const throw()
{
...
/** \brief Return a string describing the file_path2_t as a unix file path
 */
std::string	file_path2_t::to_unxpath_string()	const throw()
{
#if 0
...
/** \brief Return a string describing the file_path2_t as a win32 file path
 */
std::string	file_path2_t::to_winpath_string()	const throw()
{
 ...
 */
std::string	file_path2_t::to_os_path_string()	const throw()
{
#ifndef	_WIN32
 ...
 */
std::string	file_path2_t::to_string_sepa(const std::string &dir_separator)	const throw()
{
	std::ostringstream 	oss;
...
/** \brief serialize a file_path2_t
 * 
 * - support null file_path2_t
 */
serial_t& operator << (serial_t& serial, const file_path2_t &file_path2)	throw()
{
	// serialize each field of the object
	serial << datum_t(file_path2.to_string());
	// return serial
	return serial;
}

/** \brief unserialze a file_path2_t
 * 
 * - support null file_path2_t
 */
serial_t& operator >> (serial_t & serial, file_path2_t &file_path2)		throw(serial_except_t)
{	
	datum_t		file_path2_datum;
	// unserialize the data
	serial >> file_path2_datum;
	// set the returned variable
	file_path2	= file_path2_datum.to_stdstring();
	// return serial
	return serial;
}
