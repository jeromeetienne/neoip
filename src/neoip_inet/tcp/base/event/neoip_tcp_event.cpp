/*! \file
    \brief Implementation of the tcp_event_t

- TODO to remove the MAYSEND_OFF event
  - this is redundant and useless, query get_xmitbuf_freelen() instead
...
std::ostream & operator << (std::ostream & os, const tcp_event_t & event) throw()
{
	...
	case tcp_event_t::CNX_REFUSED:
		...
		break;
	case tcp_event_t::ITOR_TIMEDOUT:
		os << "ITOR_TIMEDOUT (reason: " << event.get_itor_timedout_reason() << ")";
		break;
...
/** \brief return true is the type is ITOR_TIMEDOUT, false otherwise
 */
bool	tcp_event_t::is_itor_timedout()						const throw()
{
	return type_val == ITOR_TIMEDOUT;
}

/** \brief set event to itor_timedout (with a possible reason)
 */
tcp_event_t tcp_event_t::build_itor_timedout(const std::string &reason)		throw()
{
	tcp_event_t	tcp_event;
	// set the type_val
	tcp_event.type_val	= ITOR_TIMEDOUT;
	tcp_event.reason_str	= reason;
	// return the built object
	return tcp_event;
}

/** \brief return the itor_timedout reason
 */
const std::string &tcp_event_t::get_itor_timedout_reason()			const throw()
{
	// sanity check - the event MUST be ITOR_TIMEDOUT
	DBG_ASSERT( is_itor_timedout() );
	// return the reasons
	return reason_str;
