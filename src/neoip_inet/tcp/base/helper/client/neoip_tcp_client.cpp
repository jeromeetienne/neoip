...
const size_t	tcp_client_t::UNLIMITED	= tcp_full_t::UNLIMITED_VAL;
...
tcp_client_t &	tcp_client_t::local_addr(const ipport_addr_t &p_local_addr)	throw()
{
	// sanity check - the tcp_client_t MUST be in itor 
	DBG_ASSERT( is_itor() );
	// set the local_addr of the tcp_itor_t
	tcp_itor->local_addr(p_local_addr);
 ...
 */
tcp_client_t &tcp_client_t::profile(const tcp_profile_t &tcp_profile)	throw()
{
	// sanity check - tcp_client_t MUST be a itor
	DBG_ASSERT( is_itor() );
	// set the profile
...
inet_err_t	tcp_client_t::start(const ipport_addr_t &remote_addr, tcp_client_cb_t *callback
						, void * userptr)		throw()
{
	return start(tcp_itor_arg_t().remote_addr(remote_addr), callback, userptr);
}

/** \brief Start the operation (via a tcp_itor_arg_t)
 */
inet_err_t	tcp_client_t::start(const tcp_itor_arg_t &itor_arg, tcp_client_cb_t *callback
						, void * userptr)		throw()
{
	inet_err_t	inet_err;
	...
	inet_err	= tcp_itor->start(itor_arg, this, NULL);
...
tcp_full_t *	tcp_client_t::steal_full()	throw()
{
	...
	// - protective measure to ensure it wont notify this tcp_client_t without noticing
