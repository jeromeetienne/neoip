/*! \file
    \brief Defintion of \ref bytearray0_t
...
const size_t	bytearray0_t::RESERVED_SIZE		= 2048;
const size_t	bytearray0_t::START_OFFSET		= 256;
const size_t	bytearray0_t::GROW_CHUNK_LEN		= 256;
 ...
 */
void	bytearray0_t::internal_ctor(const void *ptr, size_t len)		throw()
{
	size_t		reserved	= std::max(len, bytearray0_t::RESERVED_SIZE);
	// init the glib array
 ...
 */
void	bytearray0_t::internal_dtor()					throw()
{
	// sanity check - the glib_barray MUST be non null
	DBG_ASSERT( glib_barray != NULL );
 ...
 */
bytearray0_t::bytearray0_t(const size_t reserved_size, const size_t start_offset
 ...
 */
bytearray0_t &bytearray0_t::operator = (const bytearray0_t &other)			throw()
{
...
/** \brief compare 2 \ref bytearray0_t
 */
int	bytearray0_t::compare(const bytearray0_t &other)	const throw()
{
 ...
 * 
 * @return a datum containing the data of the packet, may be null if the bytearray0_t is null
 */
datum_t	bytearray0_t::to_datum(datum_flag_t flag)	const throw()
{
	// return null datum_t is the bytearray0_t is null
	if( is_null() )	return datum_t();
	// return a datum_t containing the data
	return datum_t(get_data(), get_len(), flag);
}

/** \brief Convert the bytearray0_t to a base64 string
 */
std::string	bytearray0_t::to_base64()		const throw()
{
	return base64_t::encode(get_data(), get_len());
}

/** \brief Build a bytearray0_t from a base64 encoded string
 */
bytearray0_t	bytearray0_t::from_base64(const std::string &str)	throw()
{
	return bytearray0_t( base64_t::decode(str) );
}
...
/** \brief return the len byte at the head but DONT remove them from the bytearray0_t
 */
datum_t	bytearray0_t::head_peek(size_t len)		throw(serial_except_t)
{
	// sanity check - the length MUST be >= bytearray0_t::get_len()
...
/** \brief return the len byte at the tail but DONT them from the bytearray0_t
 */
datum_t	bytearray0_t::tail_peek(size_t len)		throw(serial_except_t)
{
	// sanity check - the length MUST be >= bytearray0_t::get_len()
	if( len > get_len() )	nthrow_serial_plain("ByteArray Data too short.");
	// copy the peekd data
...
/** \brief return the len byte at the head and remove them from the bytearray0_t
 */
datum_t	bytearray0_t::head_consume(size_t len)		throw(serial_except_t)
{
...
/** \brief return the len byte at the tail and remove them from the bytearray0_t
 */
datum_t	bytearray0_t::tail_consume(size_t len)		throw(serial_except_t)
{
	// copy the consumed data
...
/** \brief set the size of the bytearray0_t
 */
bytearray0_t &	bytearray0_t::set_size(size_t len)		throw()
{
 ...
 */
bytearray0_t &	bytearray0_t::head_add(const void *data, size_t len)	throw()
{
	// add the data
 ...
 */
bytearray0_t &	bytearray0_t::head_remove(size_t len)		throw(serial_except_t)
{
	// sanity check - the length MUST be >= bytearray0_t::get_len()
 ...
 */
bytearray0_t &	bytearray0_t::tail_add(const void *data, size_t len)	throw()
{
	// add the data
 ...
 */
bytearray0_t &	bytearray0_t::tail_remove(size_t len)		throw(serial_except_t)
{
	// sanity check - the length MUST be >= bytearray0_t::get_len()
	if( len > get_len() )	nthrow_serial_plain("Data too short.");
	// if there is nothing to remove, return now
 ...
 */
std::string bytearray0_t::to_string()			const throw()
{
