/*! \file
    \brief Definition of the \ref flv_amf_parse_t class
 ...
 */
flv_err_t flv_amf_parse_t::amf_to_dvar(bytearray_t &amf_data, dvar_t &dvar_out)	throw()
{
	// log to debug
	KLOG_DBG("enter amf_data=" << amf_data);
	// call the internal parser
	try {
		dvar_out	= parser(amf_data);
 ...
 */
dvar_t	flv_amf_parse_t::parser(bytearray_t &amf_data)	throw(serial_except_t)
{
	flv_amf_type_t	amf_type;
	
	// get the flv_amf_type_t
	amf_data	>> amf_type;
	if( amf_type == flv_amf_type_t::NUMBER ){
		double	value;
		amf_data	>> value;
		// log to debug
		KLOG_DBG(amf_type.to_string() << "=" << value);
		// return it as dvar_dbl_t
		return	dvar_dbl_t(value);
	}else if( amf_type == flv_amf_type_t::BOOLEAN ){
		uint8_t	value;
		amf_data	>> value;
		// log to debug
		KLOG_DBG(amf_type.to_string() << "=" << (value ? "true" : "false"));
		// return the boolean as a int
		return	dvar_int_t(value);
	}else if( amf_type == flv_amf_type_t::STRING ){
		uint16_t	str_size;
		datum_t		str_data;
		...
		amf_data	>> highest_index;
		
		// log to debug
		KLOG_DBG(amf_type.to_string() << ": start highest_index=" << highest_index);
		// parse the array itself
		while(true){
			// if the amf_data is now empty, return the value now
			// - some .flv dont include the 0x00, 0x00, 0x09 at the end if it is 
		...
		// NOTE: this points MUST never be reached
		DBG_ASSERT( 0 );
	}else if( amf_type == flv_amf_type_t::ARRAY ){
		dvar_t	dvar	= dvar_arr_t();
		// get the number of item
		uint32_t	nb_item;
		amf_data	>> nb_item;
		// log to debug
		KLOG_DBG(amf_type.to_string() << ": start nb_item="<< nb_item);
		// parse every item and put it in the dvar_arr
		for(size_t i = 0; i < nb_item; i++)	dvar.arr() += parser(amf_data);
		// log to debug
		KLOG_DBG(amf_type.to_string() << ": end");
		// return the dvar_arr_t
		return	dvar;
	}else if( amf_type == flv_amf_type_t::DATE ){
		double		from_epoch_ms;
		uint16_t	gmt_offset_min;
		amf_data	>> from_epoch_ms;
		amf_data	>> gmt_offset_min;
		...
		nthrow_serial_plain("unparse amf_type " + amf_type.to_string() + "... given up");	
	}
}
