...
#ifndef __NEOIP_TLS_ITOR_HPP__ 
#define __NEOIP_TLS_ITOR_HPP__ 
/* system include */
/* local include */
 ...
 */
class tls_itor_t : NEOIP_COPY_CTOR_DENY
			, private wikidbg_obj_t<tls_itor_t, tls_itor_wikidbg_init> {
private:
	...
	/*************** read/write callback	*******************************/
	friend ssize_t	tls_itor_c_read_cb(void *userptr, void *buf_ptr, size_t buf_len);
	friend ssize_t	tls_itor_c_write_cb(void *userptr, const void *buf_ptr, size_t buf_len);
...
public:
	...
	/*************** Setup function	***************************************/
	tls_itor_t&	profile(const tls_profile_t &profile)	throw();
	...
	friend class	tls_itor_wikidbg_t;
};


NEOIP_NAMESPACE_END

#endif	/* __NEOIP_TLS_ITOR_HPP__  */
