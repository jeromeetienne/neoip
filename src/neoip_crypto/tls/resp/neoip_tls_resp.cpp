...
ssize_t	tls_resp_c_read_cb(void *userptr, void *buf_ptr, size_t buf_len);
ssize_t	tls_resp_c_write_cb(void *userptr, const void *buf_ptr, size_t buf_len);
 ...
 */
tls_resp_t::tls_resp_t()	throw()
{
	// zero some fields
	m_tls_privctx	= NULL;
	// init the static link http - to ease access while debugging - TODO to remove
	wikidbg_obj_add_static_page("/tls_resp_" + OSTREAMSTR(this));
}
 ...
 */
tls_resp_t &tls_resp_t::profile(const tls_profile_t &p_profile)	throw()
{
 ...
 */
crypto_err_t tls_resp_t::notify_recved_data(const pkt_t &recved_pkt
...
ssize_t	tls_resp_c_read_cb(void *userptr, void *buf_ptr, size_t buf_len)
{
	tls_resp_t *	tls_resp	= (tls_resp_t *)userptr;
	return tls_resp->cpp_read_cb(buf_ptr, buf_len);
}
...
{
	tls_resp_t *	tls_resp	= (tls_resp_t *)userptr;
	return tls_resp->cpp_write_cb(buf_ptr, buf_len);
}

/** \brief Handle the c++ read for gnutls 
 */
ssize_t	tls_resp_t::cpp_read_cb(void *buf_ptr, size_t buf_len)		throw()
{
	// log to debug
 ...
 */
ssize_t	tls_resp_t::cpp_write_cb(const void *buf_ptr, size_t buf_len)	throw()
{
	// log to debug
	KLOG_DBG("enter buf_len=" << buf_len);
