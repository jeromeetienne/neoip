...
/** \brief Constructor
 */
ntudp_socket_estarelay_testclass_t::ntudp_socket_estarelay_testclass_t()	throw()
{
	// zero some field
	peer_4client	= NULL;
	peer_4relay	= NULL;
 ...
 */
nunit_err_t	ntudp_socket_estarelay_testclass_t::neoip_nunit_testclass_init()	throw()
{
	...
	ntudp_err	= ntudp_nunit_peer_start_non_natsym(peer_4client);
	if( ntudp_err.failed() )	return nunit_err_t(nunit_err_t::ERROR, ntudp_err.to_string());

	// init the peer_4relay
	DBG_ASSERT( !peer_4relay );
	peer_4relay	= nipmem_new ntudp_peer_t();
	ntudp_err	= ntudp_nunit_peer_start_inetreach(peer_4relay, "127.0.0.1:4001");
	if( ntudp_err.failed() )	return nunit_err_t(nunit_err_t::ERROR, ntudp_err.to_string());

	// init the peer_4server
	DBG_ASSERT( !peer_4server );
	peer_4server	= nipmem_new ntudp_peer_t();
	peer_4server->set_rdvpt_ftor(ntudp_rdvpt_ftor_t(this, NULL));	
	ntudp_err	= ntudp_nunit_peer_start_non_natsym(peer_4server);
	if( ntudp_err.failed() )	return nunit_err_t(nunit_err_t::ERROR, ntudp_err.to_string());
 ...
 */
void	ntudp_socket_estarelay_testclass_t::neoip_nunit_testclass_deinit()	throw()
{
	// log to debug
	KLOG_DBG("enter");
	...
	// delete the peer_4relay
	if( peer_4relay ){
		nipmem_delete	peer_4relay;
		peer_4relay	= NULL;
	}
 ...
 */
nunit_res_t	ntudp_socket_estarelay_testclass_t::one_client(const nunit_testclass_ftor_t &testclass_ftor) throw()
{
	// launch 1 client
	ntudp_err_t	ntudp_err;
 ...
 */
nunit_res_t	ntudp_socket_estarelay_testclass_t::ten_client(const nunit_testclass_ftor_t &testclass_ftor) throw()
{
	// launch 1 client
	ntudp_err_t	ntudp_err;
 ...
 */
ntudp_err_t	ntudp_socket_estarelay_testclass_t::launch_nb_client_close(size_t nb_launch)	throw()
{
	ntudp_err_t	ntudp_err;
	...
	for(size_t i = 0; i < nb_launch; i++){
		...
		ntudp_err	= client_close->start(peer_4client, peer_4server->local_peerid()
							, this, NULL);
 ...
 */
void	ntudp_socket_estarelay_testclass_t::delete_all_client_close()	throw()
{
	// delete all the pending client_close
...
/** \brief return true if the ntudp_rdvpt_t MUST be discard, false otherwise
 * 
 ...
 */
bool ntudp_socket_estarelay_testclass_t::ntudp_rdvpt_ftor_cb(void *userptr, const ntudp_rdvpt_t &ntudp_rdvpt)	throw()
{
	// log to debug
	KLOG_DBG("ntudp_rdvpt=" << ntudp_rdvpt);
	// if the ntudp_rdvpt_t is DIRECT, reject it
	if( ntudp_rdvpt.get_type() == ntudp_rdvpt_t::DIRECT )	return true;
 ...
 */
bool ntudp_socket_estarelay_testclass_t::neoip_ntudp_socket_nunit_client_close_cb(void *cb_userptr
