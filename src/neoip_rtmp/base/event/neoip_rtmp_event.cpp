...
std::string	rtmp_event_t::to_string()	const throw()
{
	...
	case rtmp_event_t::PACKET:	oss << "PACKET(pkthd=" << pkthd << " pktdata.size="
						<< pktdata.size() <<")";	break;
	case rtmp_event_t::CONNECTED:	oss << "CONNECTED(" << connect_uri << ")";
										break;
...
/** \brief return true is the type is CONNECTED, false otherwise
 */
bool	rtmp_event_t::is_connected() const throw()
{
	return type_val == CONNECTED;
}

/** \brief build a rtmp_event_t to CONNECTED (with a possible reason)
 */
rtmp_event_t rtmp_event_t::build_connected(const http_uri_t &connect_uri)	throw()
{
	rtmp_event_t	swarm_event;
	// set the type_val
	swarm_event.type_val	= CONNECTED;
	swarm_event.connect_uri	= connect_uri;
	// return the built object
	return swarm_event;
}

/** \brief return the CONNECTED data
 */
const http_uri_t &	rtmp_event_t::get_connected_uri() const throw()
{
	// sanity check - the event MUST be CONNECTED
	DBG_ASSERT( is_connected() );
	// return the connect_uri
	return connect_uri;
}
