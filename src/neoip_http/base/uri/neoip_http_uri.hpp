/*! \file
    \brief Header of the \ref http_uri_t

*/


#ifndef __NEOIP_HTTP_URI_HPP__
#define __NEOIP_HTTP_URI_HPP__
/* system include */
#include <string>
/* local include */
 ...
 */
class http_uri_t : NEOIP_COPY_CTOR_ALLOW {
...
public:
	/*************** ctor/dtor	***************************************/
	http_uri_t()				throw()	{}
	http_uri_t(const std::string &str)	throw()	{ ctor_from_str(str);			}
	http_uri_t(const char *str)		throw()	{ ctor_from_str(std::string(str));	}
	...
	/*************** action function	*******************************/
	http_uri_t &		clear_hostport()	throw();
	http_uri_t &		clear_pathquery()	throw();
	http_uri_t &		clear_query()		throw();

	/*************** uri scramble stuff	*******************************/
	bool			is_scrambled()		const throw();
	http_uri_t		unscramble()		const throw();

	/*************** comparison operator	*******************************/
	int	compare(const http_uri_t & other)	const throw();
	bool	operator ==(const http_uri_t & other)  const throw(){ return compare(other) == 0; }
	bool	operator !=(const http_uri_t & other)  const throw(){ return compare(other) != 0; }
	bool	operator < (const http_uri_t & other)  const throw(){ return compare(other) <  0; }
	bool	operator <=(const http_uri_t & other)  const throw(){ return compare(other) <= 0; }
	bool	operator > (const http_uri_t & other)  const throw(){ return compare(other) >  0; }
	...
	friend std::ostream & operator << (std::ostream & os, const http_uri_t & http_uri) throw()
					{ return os << http_uri.to_string();	}

	/*************** serial_t function	*******************************/
	friend	serial_t& operator << (serial_t& serial, const http_uri_t &http_uri)	throw();
	friend	serial_t& operator >> (serial_t & serial, http_uri_t &http_uri) 	throw(serial_except_t);

	/*************** xmlrpc function	*******************************/
	friend	xmlrpc_build_t &operator << (xmlrpc_build_t& xmlrpc_build, const http_uri_t &http_uri)	throw();
	friend	xmlrpc_parse_t &operator >> (xmlrpc_parse_t& xmlrpc_parse, http_uri_t &http_uri)		throw(xml_except_t);
};

NEOIP_NAMESPACE_END

#endif	/* __NEOIP_HTTP_URI_HPP__  */
