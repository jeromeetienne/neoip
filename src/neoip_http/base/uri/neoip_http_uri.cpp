...
/** \brief Clear the query part of the http_uri_t
 */
http_uri_t &	http_uri_t::clear_query()	throw()
{
	// sanity check - the http_uri_t MUST NOT be null
	DBG_ASSERT( !is_null() );
	// set the 'cleared' value
...
/** \brief Return true if this http_uri_t is scrambled, false otherwise
 */
bool		http_uri_t::is_scrambled()		const throw()
{
	// log to debug
	KLOG_DBG("this=" << *this);
	...
	// if all previous test passed, this http_uri_t is considered scambled, return true
	return true;
}
/** \brief unscramble this http_uri
 */
http_uri_t	http_uri_t::unscramble()		const throw()
{
	// sanity check - the http_uri_t MUST be scrambled
	...
	// merge the var() of this http_uri_t to the unscrambled one
