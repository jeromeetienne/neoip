#!/bin/sh
#
# launch a web browser to the neoip_buddy web server to configure it
#
...
notify_user(){
	...
	RET=$?
	if [ "$RET" != "0" ]; then
		FNAME=/tmp/neoip_buddy_configurator.$$.html
		echo $MSG >$FNAME
		$BROWSER file://$FNAME
...
#
# get the address of the neoip buddy http web site
#
get_neoip_buddy_url(){
	CONFIG_DIR=/home/jerome/workspace/yavipin/src/conf_netcat1
	if [ -r $CONFIG_DIR/lib_httpd_url.conf ]; then
		URL=`head $CONFIG_DIR/lib_httpd_url.conf`
	else
...
tst_neoip_buddy_url(){
	...
	if [ "$RET" != "0" ]; then
		notify_user "NeoIP buddy url is assumed to be $URL. But it doesnt answer! <br> try to type 'service neoip_buddy start' as root "
		exit -1
	fi
}
...
get_browser
# get the http url of the buddy
get_neoip_buddy_url
# test if the url answer
tst_neoip_buddy_url
# launch the browser itself
$BROWSER $URL/neoip_lib/handler_list.html
