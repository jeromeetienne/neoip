...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
  ...
  <xsl:template match="/zi:interface">
	...
	<pre>$ 0alias <xsl:value-of select="zi:name"/><xsl:text> </xsl:text><xsl:value-of select='/zi:interface/@uri'/></pre>
	<pre>$ <xsl:value-of select="zi:name"/> --ctrl doinstall_confdir</pre>
	<pre>$ <xsl:value-of select="zi:name"/> --ctrl uninstall_confdir</pre>
	<pre>$ sudo `which <xsl:value-of select="zi:name"/>` --ctrl doinstall_extrsc</pre>
	<pre>$ sudo `which <xsl:value-of select="zi:name"/>` --ctrl uninstall_extrsc</pre>
	<p>
